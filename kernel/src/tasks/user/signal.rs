...
use super::entry::UserTaskImplContainer;

impl UserTaskImplContainer {
    pub async fn handle_signal(&mut self, task: Arc<UserTask>, signal: SignalFlags) {
        debug!(
        ...
        loop {
            ...
            if let UserTaskControlFlow::Break = self.handle_user_interrupt().await {
                break;
            }
        }
        info!(
